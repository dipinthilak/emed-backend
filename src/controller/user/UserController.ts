...
export class UserController {
    ...
    async onUserdata(req:Request,res:Response,next:NextFunction){
      try {
        const userId=req.params.userId;
        const data=await this.interactor.onUserdata(userId);
        return res.status(200).json(data)      
    ...
    async onUserUpdate(req:Request,res:Response,next:NextFunction){
      try {
        const userId=req.params.userId;
        const {_id,phoneNo,gender,address}=req.body;   
        const input={_id,phoneNo,gender,address};    
        const data=await this.interactor.onUserUpdate(_id,input);
        return res.status(200).json(data)      
    ...
    async onAddMember(req:Request,res:Response,next:NextFunction){
      try {
        const userId=req.params.userId;
        console.log("user id ---->",userId);
        console.log("member details-->",req.body);
        console.log("checking datas----");
        
        return res.status(200).json({status:false,message:"data at controller"})      
